...
function PANEL:SetItem(itemTable)
	...
	self.price:SetText(itemTable.price and ix.currency.Get(itemTable.price) or ix.currency.Get(ix.config.Get("TerminalDefaultPrice",10)))
	...
	self.icon.DoClick = function(this)
		if itemTable.category:match(LocalPlayer():GetCharacter():GetData("terminal_licence","-----")) then
		...
		Derma_Message( "You can't buy this item without licence on that category!", "Buy item", "OK")
	end
...
function PANEL:Init()
	...
	for k, v in pairs(ix.item.list) do
		if !PLUGIN.LicencesWhichPlayersCantBuy[v.category] then
...
function PANEL:LoadItems(category, search)
	...
	for uniqueID, itemTable in SortedPairsByMemberValue(items, "name") do
		if !PLUGIN.LicencesWhichPlayersCantBuy[itemTable.category] then
...
function PANEL:OnQuantityChanged()
	...
	for k, v in pairs(self.itemData) do
		...
		if (itemTable and v > 0) then
			valid = valid + 1
			price = price + (v * (itemTable.price or ix.config.Get("TerminalDefaultPrice",10))) --ddddd
		end
	end
	...
	for k, v in SortedPairs(items) do
		...
		if (itemTable) then
			...
			slot.name:SetText(string.format(
				...
				itemTable.price and ix.currency.Get(itemTable.price) or ix.config.Get("TerminalDefaultPrice",10)
			))
