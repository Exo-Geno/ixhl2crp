...
if (SERVER) then
	...
	net.Receive("ixBusinessBuy", function(length, client)
		...
		for k, v in pairs(items) do
			...
			if itemTable.category:match(char:GetData("terminal_licence","-----")) then 
				if (itemTable) and !PLUGIN.LicencesWhichPlayersCantBuy[itemTable.category] then
					...
					if (amount == 0) then
						...
						cost = cost + (amount * (itemTable.price or ix.config.Get("TerminalDefaultPrice",10)))
					end
				else
					items[k] = nil
				end
		...
		if (char:HasMoney(cost)) then
			...
			entity:Spawn()
			local pos = IsValid(client.aw_terminal) and (client.aw_terminal:GetPos() + Vector(100,0,20)) or client:GetPos()
			print(client.aw_terminal)
			entity:SetPos(pos)
